...
gfile='/etc/group'
hdir='/home'
lastchg=$(( $(date +%s) / 86400 )) # number of days since Jan. 1, 1970
...
echo -n "login: "   ; read -r login # -n: ensure the echo command does not print a new line, so the read command input stays the same line
...
# find the largest UID under 5000 and use the next integer
...
echo "${login}:*:${lastchg}:0:99999:7:::" >> $shadowfile # add the user’s password-related entry to /etc/shadow, * indicates no password is currently set
...
mkdir "$homedir"
# cp -a /etc/skel/. "$homedir" # copy all files from /etc/skel to the home directory
